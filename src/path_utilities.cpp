...
Trajectory::Trajectory(MPC &mpc_)
{
    ...
    // traj_sub = nh.subscribe<nav_msgs::Path>("//move_base/NavfnROS/plan", 10, &Trajectory::trajCallback, this);
    ...
    // odom_sub = nh.subscribe<nav_msgs::Odometry>("/tricycle_controller/odom", 10, &Trajectory::odomCallback, this);

    last_steer_cmd = 0;
    last_throttle_cmd = 0;
}

void Trajectory::subscribeToTopics()
{
    if(DEBUG) ROS_INFO("Subscribing");
    traj_sub = nh.subscribe<nav_msgs::Path>("/move_base/NavfnROS/plan", 10, &Trajectory::trajCallback, this);
    amclpose_sub = nh.subscribe<geometry_msgs::PoseWithCovarianceStamped>("/amcl_pose", 10, &Trajectory::carPoseCallback, this);
    odom_sub = nh.subscribe<nav_msgs::Odometry>("/tricycle_controller/odom", 10, &Trajectory::odomCallback, this);

    // nav_msgs/OccupancyGrid /move_base/local_costmap/costmap
    localcostmap_sub = nh.subscribe<nav_msgs::OccupancyGrid>("/move_base/local_costmap/costmap", 10, &Trajectory::localcostmapCallback, this);
 
    localcostmapUpdate_sub = nh.subscribe<map_msgs::OccupancyGridUpdate>("/move_base/local_costmap/costmap_updates", 10, &Trajectory::localcostmapUpdateCallback, this);
...
{
    nav_msgs::Path retrievedPath = *trajectoryMsg;
    ...
    // if(trajectoryMsg != nullptr)
    {
        ...
        globalPath = retrievedPath;
        ROS_INFO("Retrieved path: %d points ...", globalPath.poses.size());
    }
    if(globalPath.poses.size() != retrievedPath.poses.size())
    {
        ROS_INFO("Retrieved NEW global trajectory...");
        globalPath = retrievedPath;
        ROS_INFO("Retrieved path: %d points ...", globalPath.poses.size());
    }
}
...
void Trajectory::carPoseCallback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr &amclPoseMsg)
{
    ...
    ROS_INFO("globalPath.poses.size() %d",globalPath.poses.size());
    if (globalPath.poses.size() > 0)
    {
...
void Trajectory::predictControl()
{
    if (coeffsLocalSet)
    {
        ...
        mpc_msg.linear.x = vars[1];
        mpc_msg.angular.z = -vars[0];
        // cmd_pub.publish(ctrl_msg);
        ROS_INFO("A:[%f] Steer:[%f]",vars[1], vars[0]);
    ...
    if (globalPath.poses.size() != 0)
    {
        ...
        for (int i = 0; i < coeff_N; i++)
        {
            ...
            /*
                 ...
                 */
            tf::Pose tfPath;
            // if(i*stepSize < globalPath.poses.size())
            {
                // globalPath.poses[point + (i*stepSize)].pose = globalPath.poses[globalPath.poses.size()].pose;
                tf::poseMsgToTF(globalPath.poses[point + (i*stepSize)].pose, tfPath);
                tf::Pose tfCar;
...
void Trajectory::localcostmapCallback(const nav_msgs::OccupancyGrid::ConstPtr &localcostmapMsg)
{
    ROS_INFO("--------- Local Cost Map Callback ---------");
    occupanyGrid = localcostmapMsg->data;
    // ROS_INFO("occupanyGrid.size(): %ld", occupanyGrid.size());
    ROS_INFO("origin x: %f y: %f", localcostmapMsg->info.origin.position.x, localcostmapMsg->info.origin.position.y);
    ROS_INFO("width x: %d y: %d", localcostmapMsg->info.width, localcostmapMsg->info.height);
    map_width = localcostmapMsg->info.width;
    map_height = localcostmapMsg->info.height;

    ROS_INFO("occupanyGrid.size(): %ld", occupanyGrid.size());
    map_org_x = localcostmapMsg->info.origin.position.x;
    map_org_y = localcostmapMsg->info.origin.position.y;

    map_resolution = localcostmapMsg->info.resolution;
    ROS_INFO("resolution: %f", localcostmapMsg->info.resolution);
    ROS_INFO("--------- Local Cost Map Callback ---------");

}

void Trajectory::localcostmapUpdateCallback(const map_msgs::OccupancyGridUpdate::ConstPtr &localcostmapUpdateMsg)
{
    occupanyGrid_updated = localcostmapUpdateMsg->data;
    // ROS_INFO("occupanyGrid_updated.size(): %ld", occupanyGrid_updated.size());

}

void Trajectory::calculateLocalPlanningCost()
{
    // std::string footprint_str = "/move_base/local_costmap/footprint";
    // std::string xxx;

    // if(ros::param::has(footprint_str.c_str()))
    // {
    //     ros::param::get(footprint_str.c_str(), xxx);
    //     // ROS_INFO("xxx: %s", xxx);
    //     auto footprint_vec_str = xxx;
    //     std::cout << xxx << std::endl;
        
    //     footprint_vec_str = footprint_vec_str.substr(1, footprint_vec_str.size()-2);
    //     std::cout << "footprint_vec_str.type() " << footprint_vec_str.data() << std::endl;

    //     // std::vector< std::vector<float> > footprint_vec(4,0);
    //     auto footprint_vec{footprint_vec_str};
    //     std::cout << footprint_vec << std::endl;
    // }

    std::vector< std::vector<float> > footprint = {{0.42,-0.225},{0.42,0.225},{-0.81,0.225},{-0.81,-0.225}};

    float robot_pos_x = poseCar.position.x;
    float robot_pos_y = poseCar.position.y;
    ROS_INFO("robot_pos_x: %f robot_pos_y: %f", robot_pos_x, robot_pos_y);
    ROS_INFO("map_resolution: %f", map_resolution);

    int cell_x = (robot_pos_x / map_resolution) + map_width/2;
    int cell_y = (robot_pos_y / map_resolution) + map_height/2;

    ROS_INFO("cell_x: %d cell_y: %d", cell_x, cell_y);

    if(occupanyGrid_updated.size() > 0)
    {
        ROS_INFO("occupanyGrid_updated %f", occupanyGrid_updated[cell_x, cell_y]);
    }
