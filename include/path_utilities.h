...
private:
    ...
    ros::Subscriber localcostmapUpdate_sub;
    ...
    std::vector<signed char> occupanyGrid_updated;
    std::vector<signed char> occupanyGrid;
    ...
    geometry_msgs::Twist mpc_msg;

    tf::Quaternion q_car_global;

    double last_steer_cmd, last_throttle_cmd;

    const double Lf = 0.7; //TODO: make this global parameter
...
public:
    ...
    void localcostmapCallback(const nav_msgs::OccupancyGrid::ConstPtr &localcostmapMsg);

    void localcostmapUpdateCallback(const map_msgs::OccupancyGridUpdate::ConstPtr &localcostmapUpdateMsg);

    void calculateLocalPlanningCost();
